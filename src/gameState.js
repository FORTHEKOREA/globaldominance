...
import { computeUpgradeCost, recruitCost } from './game/military.js';

const AI_COUNT = 30;
const AI_IDS = Array.from({ length: AI_COUNT }, (_, i) => `enemy${i + 1}`);
...
export const state = {
    ...
    enemies: AI_IDS,
    enemyModLevels: Object.fromEntries(AI_IDS.map(id => [id, 1]))
};
...
export function assignInitialFactions(playerStartId) {
    ...
    // Give 3 regions to each enemy to support larger AI counts.
    let i = 0;
    ...
    setInterval(() => {
        ...
        for (const [id, region] of Object.entries(state.regions)) {
            if (region.owner === 'player') {
                ...
                income += (region.population * region.gdp) / 3600 * 0.5;
            }
...
export function getUpgradeCost() {
    ...
    return computeUpgradeCost(totalTroops, avgGdp, state.playerModLevel);
}
...
export function sellTerritory(regionId) {
    const region = state.regions[regionId];
    if (!region) return { success: false, msg: 'Invalid territory.', type: 'warning' };
    if (region.owner !== 'player') {
        return { success: false, msg: 'You can only sell your own territory.', type: 'danger' };
    }

    const price = Math.max(0, Math.floor(region.population * region.gdp * 0.7));
    state.playerMoney += price;
    region.owner = 'neutral';
    region.troops = Math.max(1, Math.floor(region.population * 0.01));

    notify();
    return {
        success: true,
        msg: `Sold ${region.name} for ${formatCurrency(price)}.`,
        type: 'success',
        newFunds: state.playerMoney,
    };
}

export function recruitTroops(regionId, unitsOfTenThousand = 1) {
    const region = state.regions[regionId];
    if (!region) return { success: false, msg: 'Invalid territory.', type: 'warning' };
    if (region.owner !== 'player') {
        return { success: false, msg: 'You can only recruit in your own territory.', type: 'danger' };
    }

    const units = Math.max(1, Math.floor(unitsOfTenThousand));
    const cost = recruitCost(state.playerModLevel, units);
    if (state.playerMoney < cost) {
        return { success: false, msg: 'Insufficient funds.', type: 'danger' };
    }

    state.playerMoney -= cost;
    region.troops += units * 10_000;
    notify();
    return {
        success: true,
        msg: `Recruited ${(units * 10_000).toLocaleString()} troops.`,
        type: 'success',
        cost,
        newFunds: state.playerMoney,
    };
